...
\maketitle
\begin{abstract}
How sensitive are conclusions about adversarial transfer to direction, perturbation budget, and source-success conditioning? We remeasure established transfer phenomena on the full CIFAR-10 test set using three independently trained ResNet-18, VGG16, and MobileNetV2 checkpoints. The corrected evaluation excludes existing clean errors and reports pairwise transfer (PTR), transfer conditional on source success (CTR), and exact denominators. At $8/255$, mean PGD PTR ranges from 18.09\% for ResNet-to-VGG to 95.23\% for VGG-to-MobileNet; at $2/255$, those rates fall to 5.84\% and 23.29\%. We decompose PTR exactly into successful-source and failed-source strata. For VGG-sourced $2/255$ PGD, target error among eligible source failures is only 0.42--0.96\% toward ResNet and 2.71--2.99\% toward MobileNet. Thus, most of the PTR--CTR gap follows mechanically from incomplete source success, while the nonzero failed-source term prevents deriving CTR from the conventionally reported ASR and PTR alone. A seed-0 control training all architectures at learning rate 0.01 preserves the VGG transfer asymmetry: its outbound PTR is 93.12--95.98\%, versus 18.55--32.71\% in reverse. The contribution is a seed-replicated, denominator-complete measurement of known direction and optimization effects, together with a transparent account of what source conditioning adds.
\end{abstract}
...
Directionality and the tension between white-box optimization and transfer are established phenomena. Demontis et al. explain asymmetric transfer through properties including target vulnerability and surrogate complexity \cite{demontis}. Dong et al. introduce momentum to stabilize iterative attack directions and improve black-box transfer \cite{dong}. Our contribution is not the rediscovery of asymmetry or source overfitting. We instead provide a seed-replicated remeasurement with explicit jointly clean populations, source-success strata, and exact counts, and quantify when CTR is almost determined by source success and when a failed-source residual remains.
...
Repository commit \texttt{58f8102} compared CIFAR-10 labels directly with the indices of 1,000-class ImageNet models, omitted required preprocessing, mixed perturbation coordinate systems, and counted existing clean errors as attack successes. It also lacked compatible checkpoints and per-example logs. Those rates and rankings are excluded from the revised empirical record. This self-correction motivates compatible labels, raw-pixel budgets, clean eligibility, and recorded source success; it is not the empirical contribution. The line-by-line audit is retained in the supplementary artifact.
...
The matched-learning-rate control preserves the two largest VGG asymmetries (Table~\ref{tab:lrcontrol}). VGG16-to-ResNet-18 exceeds the reverse direction by 63.27 points, and VGG16-to-MobileNetV2 exceeds the reverse by 74.57 points. The control ResNet-18 has 93.82\% clean test accuracy, compared with 95.11\% for its primary seed-0 checkpoint; control MobileNetV2 has 90.90\%, compared with 89.97\%. Individual transfer entries change by up to 25.93 points across the full matrix, so the check rejects a learning-rate-only explanation for the VGG directionality but does not isolate architecture as its cause.
...
At $2/255$, VGG16 source ASR is only 71.05--72.98\% (Table~\ref{tab:asr}). Table~\ref{tab:conditioning} applies the exact conditioning identity. Eligible VGG source failures retain target error of 0.42--0.96\% toward ResNet-18 and 2.71--2.99\% toward MobileNetV2. Thus, incomplete pairwise source success explains most of the PTR--CTR gap, but the failed-source residual is nonzero and larger toward MobileNetV2. The conventional source ASR and PTR do not determine CTR because ASR uses a broader denominator and omits this residual. For four other directions the residual is zero or at most 0.36\%, making CTR nearly equal to PTR divided by pairwise source success.

\begin{table}[t]
\centering
\caption{$2/255$ PGD conditioning decomposition, mean [minimum, maximum] percentage over three seeds. Pair ASR is $a_{st}$ and Fail-target is $b_{st}$.}
...
\caption{Left: mean PGD PTR over three seeds at each budget, with minimum-to-maximum seed bars. Right: observed CTR versus the zero-residual approximation $\mathrm{PTR}/a_{st}$ for PGD at $2/255$; labels above bars give mean failed-source target error $b_{st}$. R, V, and M denote ResNet-18, VGG16, and MobileNetV2.}
\label{fig:summary}
\end{figure*}
}{}
...
\section{Conclusion}
Across three training seeds, adversarial transfer on CIFAR-10 is strongly directional and budget dependent. At $8/255$, mean PGD PTR spans 18.09--95.23\% across off-diagonal pairs; at $2/255$, it spans 4.61--23.29\%. The exact conditioning decomposition shows why PTR and CTR separate: target errors are uncommon but not universally absent when the source attack fails. Reporting pairwise source success and the failed-source stratum distinguishes this mechanical conditioning effect from additional transfer behavior. The strongest VGG directionality persists in a seed-0 matched-learning-rate control, while the movement of individual entries cautions against an architecture-only interpretation. Stronger source optimization can also reduce transfer for a particular direction, replicating established source-overfitting behavior. Compatible labels and preprocessing make these measurements meaningful; clean-error exclusions, explicit denominators, seed-level reporting, and convergence checks make their limits visible.
