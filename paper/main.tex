...
\begin{table}[t]
\centering
\caption{PGD PTR (\%) at $8/255$. Entries are the three-seed mean [envelope of seed-specific Wilson 95\% intervals]. This interval envelope is descriptive and does not pool repeated test images.}
\label{tab:primary}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lrrr}
\toprule
...
\midrule
ResNet-18 & --- & 18.09 [15.97,20.22] & 72.27 [70.81,73.64]\\
VGG16 & 92.73 [91.83,93.50] & --- & 95.23 [94.31,96.02]\\
MobileNetV2 & 67.60 [62.45,71.55] & 25.94 [24.81,27.23] & ---\\
\bottomrule
\end{tabular}
}
\end{table}
...
At $2/255$, VGG16 source ASR is only 71.05--72.98\% (Table~\ref{tab:asr}). Table~\ref{tab:conditioning} applies the exact conditioning identity with absolute counts. Conditioning is nearly mechanical in five directions: $b_{st}$ is at most 0.68\%, and observed CTR is within 0.36 points of $\mathrm{PTR}/a_{st}$. VGG16-to-MobileNetV2 is the exception, with mean $b_{st}=2.83\%$ and observed CTR 1.17 points below the zero-residual value. This exception confirms that the successful- and failed-source strata are not interchangeable. More fundamentally, conventional source ASR and PTR cannot determine CTR even when the residual is negligible, because conventional ASR uses a broader, target-independent denominator than $a_{st}$.
