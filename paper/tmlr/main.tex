...
\maketitle
\begin{abstract}
Conventionally reported source attack success rate (ASR) and pairwise transfer rate (PTR) cannot reconstruct transfer conditional on source success (CTR): source ASR uses a broader denominator than the pair-dependent population required by CTR, and it omits target behavior when the source attack fails. We demonstrate this distinction on the full CIFAR-10 test set using three independently trained ResNet-18, VGG16, and MobileNetV2 checkpoints. The corrected evaluation excludes existing clean errors and reports exact denominators. At $2/255$, conditioning is nearly mechanical in five of six directions: target error among eligible source failures is at most 0.68\%, and mean CTR is within 0.27 points of the zero-residual value $\mathrm{PTR}/a_{st}$. VGG16-to-MobileNetV2 is the exception, with a 2.83\% failed-source residual and a 1.19-point difference. At $8/255$, mean PGD PTR ranges from 18.09\% for ResNet-to-VGG to 95.23\% for VGG-to-MobileNet. A seed-0 control training all architectures at learning rate 0.01 preserves the VGG transfer asymmetry: its outbound PTR is 93.12--95.98\%, versus 18.55--32.71\% in reverse. The result is a seed-replicated, denominator-complete account of when source conditioning adds information and when it acts mainly as arithmetic rescaling.
\end{abstract}

\section{Introduction}
Adversarial examples expose a gap between ordinary predictive accuracy and behavior under deliberately chosen perturbations. FGSM provides a gradient-based construction \citep{goodfellow}, while iterative optimization enables broader searches within a threat model \citep{madry,carlini}. Transfer attacks use a source model to construct an input that is subsequently evaluated on another model. The target supplies neither gradients nor feedback during construction.
...
The study asks whether conclusions about attack rankings and model-pair differences are stable under explicitly stated conditioning populations. Its contributions are:
\begin{enumerate}
\item an exact decomposition showing why conventional source ASR and PTR do not determine CTR;
\item a full-test, three-seed measurement with jointly clean populations, exact counts, and PGD PTR spanning 18.09--95.23\% at $8/255$; and
...
\end{enumerate}
The self-correction in Section 3 motivates the measurement question and is not evidence of a general failure rate across the literature.
...
An earlier implementation compared CIFAR-10 labels directly with the indices of 1,000-class ImageNet models, omitted required preprocessing, mixed perturbation coordinate systems, and counted existing clean errors as attack successes. It also lacked compatible checkpoints and per-example logs. Those rates and rankings are excluded from the revised empirical record. This self-correction motivates compatible labels, raw-pixel budgets, clean eligibility, and recorded source success; it is not the empirical contribution. The line-by-line audit is retained in the anonymized supplementary artifact.
